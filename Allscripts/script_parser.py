...
def split_files(filename):
  ...
  for line in f_read:
    ...
    line_match = re.match(r"[0-9| |.|\t]+(int.|ext.|int --|ext --)+", line.strip())
    line_match_end = re.match(r"^(the end|fade to black\.|fade out\.|freeze frame|the end\.)$", line.strip())
    ...
    if (line_match_end != None or
        line.strip().startswith('writers : ')):
      break
...
# This method performs the tokenization (i.e. punctuation removal and stemming)
# of a line of text.
def tokenize(line):
...
# This method writes to allwords.txt
def write_allwords():
